	...
	public static void clickon_ListElement_indexbased(List<WebElement> element, int num) {
		try {
			...
			if (desiredElementIndex >= 0 && desiredElementIndex < element.size()) {
				WebElement webElement = element.get(desiredElementIndex);
				jsScrollDownElement(webElement);
				sleep(1000);
				webElement.click();
...
//			}
	...
	public static String generateRandomEmail() {
		...
		for (int i = 0; i < 1; i++) {
			...
			middleNumber += digit;
			String email = "poovarasan+cap+" + middleNumber + "@mindwaveventures.com";
			Email = email;
			EmailId += email;
		}
		return EmailId;
	...
	public static void Random_value_storedin_Excel(String path, String sheetTitle, int Rownum, int Cellnum1,
			int Cellnum2, int Cellnum3) throws Throwable {
...
//	Only for MSI project usage
	...
	public static void DateofBirth(String datemonthyear, String dynamic_datemonthyear) {

		List<WebElement> datemonthyear_options = driver
				.findElements(By.xpath("//div[@class='dateofbirth-head']/select/option[normalize-space()='"
						+ datemonthyear + "']/following-sibling::option"));
		for (WebElement single_datemonthyear : datemonthyear_options) {
			if (single_datemonthyear.getText().equals(dynamic_datemonthyear)) {
				single_datemonthyear.click();
				System.out.println("Successfully click on the dynamic text");
			} else {
				System.out.println("Dynamic text is not valid");
			}

		}
	}

	public static void ethnicity_selection(String ethnicity_type) {

		WebElement findElement = driver
				.findElement(By.xpath("//div[contains(text(),'" + ethnicity_type + "')]/preceding-sibling::div/input"));
		sleep(1000);
	...
	public static void select_typesofex(String sextypes, int index) {
		try {
			driver.findElement(By.xpath("//input[@aria-label='" + sextypes + "']")).click();
			wait(20);
			List<WebElement> sextypes_buttons = driver.findElements(
					By.xpath("//input[@aria-label='" + sextypes + "']/parent::div/following-sibling::div/div/button"));

			if (index >= 0 && index < sextypes_buttons.size()) {
				sextypes_buttons.get(index).click();

				System.out.println("Successfully click the specific index");
			} else {
	...
	public static void gender_selection(String gendertype) {

		driver.findElement(By.xpath("//div[@class='pad-te']/div/button[contains(text(),'" + gendertype + "')]"))
				.click();
	}
